...
export async function approveCancellation(campaignId: string): Promise<void> {
  const response = await authedFetch(`/v1/campaigns/${campaignId}/approve-cancellation`, {
    method: 'POST',
  })
  if (!response.ok) throw new Error(`HTTP ${response.status}`)
}

export async function denyCancellation(campaignId: string): Promise<void> {
  const response = await authedFetch(`/v1/campaigns/${campaignId}/deny-cancellation`, {
    method: 'POST',
  })
  if (!response.ok) throw new Error(`HTTP ${response.status}`)
}
