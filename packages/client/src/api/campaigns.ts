...
export async function verifyMilestone(
  ...
  if (!response.ok) throw new Error(`HTTP ${response.status}`)
  const json = await response.json()
  return (json as { data: VerifyMilestoneResult }).data
}
...
// TODO: denyCancellation — POST /v1/campaigns/:id/deny-cancel endpoint does not exist on the
// server yet. Add server-side implementation before enabling this function.
