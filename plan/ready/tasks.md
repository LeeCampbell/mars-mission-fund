...
- [x] TASK-02: Update specs/tech/architecture.md — fix server/ path in Section 1
  ...
  - **Details**: In `specs/tech/architecture.md` Section 1 (Purpose), update the local demo scope blockquote. Change the server path from `server/` to `packages/server/`. Change the run command from `cd server && npm run dev` to `npm run dev:server` from the repo root. Bump the spec version from `0.4` to `0.5` in the frontmatter. Add a change log entry for today's date (2026-03-10) summarising the path correction.
...
- [x] TASK-03: Update specs/tech/tech-stack.md — fix Local Development section
  ...
  - **Details**: In `specs/tech/tech-stack.md`, make these changes to the Local Development section:
    ...
    1. Express Server run command: change `cd server && npm run dev` to `npm run dev:server` (run from repo root).
    1. Server Directory Layout code block: change the root label from `server/` to `packages/server/`.
...
- [x] TASK-04: Update specs/README.md — add specs/adrs/ to layout and tooling table
  ...
  - **Details**: In `specs/README.md`:
    ...
    1. Tooling table: add a new row for ADRs — document `adrs/` (or `adrs/0001-npm-workspaces-monorepo.md`), purpose "Architecture Decision Records capturing technology and structural choices."
