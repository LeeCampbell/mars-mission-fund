...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (a === '--repeat-window-sec') out.repeatWindowSec = parseInt(argv[++i], 10) || out.repeatWindowSec;
    else if (a === '--oscillation-window-sec') out.oscillationWindowSec = parseInt(argv[++i], 10) || out.oscillationWindowSec;
    else if (a === '--permission-window-sec') out.permissionWindowSec = parseInt(argv[++i], 10) || out.permissionWindowSec;
    else if (a === '--max-line-length' || a === '--max-len') out.maxLineLength = parseInt(argv[++i], 10) || out.maxLineLength;
...
function emitThrashIfNeeded(state) {
  ...
  state.actions = state.actions.filter((x) => nowMs - x.t <= Math.max(repeatWindow, oscillationWindow));
  ...
  if (recentPerms.length >= 3) {
    const joined = recentPerms.map((x) => x.detail).slice(-3).join(' | ');
...
function maybeClassifyDecision(text) {
  ...
  const decisionHints = [
    'i will', 'i’ll', 'next i', 'my plan', 'i am going to', 'i’m going to',
    'i should', 'i need to', 'let me', 'first,', 'first ', 'then ', 'to fix this',
    'the issue is', 'root cause', 'workaround', 'trying a different', 'switching to'
  ];
...
function maybeClassifyStatus(text) {
  const s = compactText(text).toLowerCase();
  const statusHints = [
    'checking', 'investigating', 'running', 'reading', 'searching', 'updating',
    'testing', 'validating', 'reproducing', 'inspecting', 'reviewing', 'comparing'
  ];
...
function extractTextFromContent(content) {
  ...
  for (const item of content) {
    ...
    if (item.type === 'tool_use') parts.push({ kind: 'tool_use', tool: item.name, input: item.input, id: item.id });
    if (item.type === 'thinking' && item.thinking) parts.push({ kind: 'thinking', text: item.thinking });
  }
  return parts;
}
...
function tryParsePermissionMessage(text) {
  ...
  if (/permission/i.test(s) || /haven't granted/i.test(s) || /not allowed/i.test(s) || /denied/i.test(s)) {
    return s;
  }
  return null;
}
...
function handleToolUse(state, toolName, input, toolId, parentToolUseId) {
  ...
  addAction(state, normalizeAction(toolName || 'tool', inputText), { toolName, input, toolId, parentToolUseId });
  emitThrashIfNeeded(state);
}
...
function handleToolResult(state, toolUseId, content, isError, parentToolUseId) {
  ...
  addAction(state, normalizeAction(isError ? 'tool_error' : 'tool_result', text), { toolUseId, text, isError, parentToolUseId });
  emitThrashIfNeeded(state);
}
...
function handleRecord(state, record, rawLine) {
  ...
  if (record.type === 'system' && record.subtype === 'init') {
    ...
    const tools = Array.isArray(record.tools) ? record.tools.length : (record.mcp_servers ? Object.keys(record.mcp_servers).length : '');
  ...
  if (record.type === 'stream_event') {
    ...
    const messageId = (ev.message && (ev.message.id || ev.message.message_id)) || record.message_id || 'stream';
    const index = ev.index != null ? ev.index : ((ev.content_block && ev.content_block.index != null) ? ev.content_block.index : 0);
    ...
    if (evType === 'content_block_start') {
      ...
      if (cb.type === 'tool_use') {
        ...
        handleToolUse(state, cb.name, input, cb.id, record.parent_tool_use_id || record.parentToolUseId);
      }
      return;
    }
  ...
  const fallbackText = [record.type, record.subtype, record.message, record.error].filter(Boolean).map(String).join(' ');
